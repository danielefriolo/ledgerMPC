...
public class DataOnFile extends Blockchain<String>{
	...
	//private List<Message> receivedBroadcastMessages;

	public DataOnFile () throws UnknownHostException {
		this.inputCounter = 0;
		this.outputCounter = 0;
		this.receivedMessages = new HashMap<Integer, Message>();
		//this.receivedBroadcastMessages = new ArrayList<Message>();
		this.called = false;
	...
	public void writeData(byte[] message, int msgNum) throws IllegalArgumentException {
		if(called) {
			...
			byte[] sessionID = super.sid;
			int sendingPartyID = super.sender;
			int receivingPartyID = super.receiver;
	...
	public byte[] readData(int msgNum, Boolean quickness){
		if(called) {
			...
			message = protocol.prepareMessage(super.sid, sender, receiver, inputCounter, protocol.read, dummyData);
			try {
				os.write(message);
				os.flush();
			} catch (IOException e) {
	...
	/**
	 * Read data from the remote server given a communication section identifier, the identifier of the receiver and the message index 
	 ...
	 */
	/*
	 * @Override public byte[][] readBroadcastData(int msgNum, int partyNum, Boolean
	 * quickness){ byte[][] returnedMsg = new byte[partyNum][]; for (byte[] ret:
	 * returnedMsg) { ret = null; }
	 * 
	 * if(called) { byte[] message; DataInputStream is = null; DataOutputStream os =
	 * null; byte[] dummyData = {0,0,0,0};
	 * 
	 * Message msg = null; Integer readMsgNum = protocol.impossibleVal;
	 * 
	 * try { os = new DataOutputStream(clientSocket.getOutputStream()); } catch
	 * (IOException e1) { e1.printStackTrace(); }
	 * 
	 * for(Message broadcastMsg: receivedBroadcastMessages) { if
	 * (broadcastMsg.getMsgNum() == msgNum) {
	 * returnedMsg[broadcastMsg.getSendingPid()-1] = broadcastMsg.getMsg();
	 * receivedBroadcastMessages.remove(broadcastMsg); } }
	 * 
	 * for (int i = 0; i < partyNum;) { if (returnedMsg[i] == null && i != sender -
	 * 1) { message = protocol.prepareMessage(super.sid, i+1, receiver, msgNum,
	 * protocol.read, dummyData); try { os.write(message); os.flush(); } catch
	 * (IOException e) { e.printStackTrace(); }
	 * 
	 * try { is = new DataInputStream(clientSocket.getInputStream()); } catch
	 * (IOException e) { e.printStackTrace(); }
	 * 
	 * while(msgNum != readMsgNum){ msg = protocol.readSingleMessage(is); readMsgNum
	 * = msg.getMsgNum();
	 * 
	 * if(readMsgNum != protocol.impossibleVal) {
	 * if(!receivedBroadcastMessages.contains(msg)) {
	 * receivedBroadcastMessages.add(msg); } }else{ try { Thread.sleep(100); } catch
	 * (InterruptedException e) { e.printStackTrace(); } } }
	 * returnedMsg[msg.getSendingPid()-1] = msg.getMsg();
	 * receivedBroadcastMessages.remove(msg); if(returnedMsg[i] != null) { i++; }
	 * }else { i++; } } return returnedMsg; } return null; }
	 */
